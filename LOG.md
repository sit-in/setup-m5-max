...
**实际输出**：
```
yourname@MacBook-Pro ~ % xcode-select --install
...
**命令 + 实际输出**：
```bash
yourname@MacBook-Pro ~ %   xcode-select -p
/Library/Developer/CommandLineTools

yourname@MacBook-Pro ~ % git --version
...
1. **`sed: /Users/yourname/.zprofile: No such file or directory`**
...
| id_* 私钥存在 | 同上 | 同上 |
| git user.email 已设 | 未配置 | `git config --global user.email your-email@example.com` |
...
## 2026-04-23 — Tailscale SSH 连接排查 ✅

**问题**：iPhone 通过 Tailscale 能连其他 MacBook（Air / Mac Mini），但连不上这台 M5 Max。

**排查过程**：

1. **检查 Tailscale 状态** → 正常
   ```
   100.x.x.71       your-macbook-pro     active
   100.x.x.70       your-iphone          active; direct
   100.x.x.115      your-mac-mini        active
   100.x.x.89       your-macbook-air     active; direct
   ```
   Tailscale 网络本身没问题，iPhone 和 M5 Max 都在线且能互相发现。

2. **检查端口 22** → **未监听**
   ```bash
   nc -z -w2 100.x.x.71 22   # 失败，端口不通
   lsof -iTCP:22 -sTCP:LISTEN  # 无结果，没有进程监听
   ```
   只有 `com.openssh.ssh-agent`（SSH 客户端）在运行，`sshd`（SSH 服务端）没有启动。

3. **根因**：macOS 新机器默认**不开启远程登录（Remote Login）**，所以 sshd 没跑，22 端口没人监听。其他 MacBook 之前手动开过，所以能连。
...
或：系统设置 → 通用 → 共享 → 开启"远程登录"

**验证**：
```bash
nc -z -w2 100.x.x.71 22
# Connection to 100.x.x.71 port 22 [tcp/ssh] succeeded!
```

**坑总结**：
- macOS 新机器默认不开 SSH，装完系统后需要手动开启
- 其他机器能连是因为之前已经开过，不是 Tailscale 的问题
- 检查顺序：先看 Tailscale 状态 → 再看目标端口是否监听 → 最后查服务是否启用
