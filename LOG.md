...
✅ MLX + mlx-lm + mlx-vlm (uv venv, Python 3.12, GPU device 确认)
✅ HuggingFace CLI (uv tool install)
...
`ollama pull llama3.3:70b-instruct-q4_K_M` 卡在 "pulling manifest" 阶段（一直转圈），原因和 Lark 一样：
- Ollama registry（registry.ollama.ai）需要走代理
- 终端没设 `HTTP_PROXY` / `HTTPS_PROXY` / `OLLAMA_PROXY` 环境变量
- 系统代理对 CLI 工具无效
...
也可以设 Ollama 专属代理：
```bash
launchctl setenv OLLAMA_PROXY http://127.0.0.1:7890
