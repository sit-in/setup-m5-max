...
真正的第 -1 步：装代理工具（魔法）
       ↓
然后才是装 ToDesk / Xcode CLT / git clone / Homebrew / ...
```

**为什么这是第一步**：
- 现代开发工具链 90% 的依赖都在 GitHub / 其他海外源
- 不装代理 = 每装一个工具都要单独找国内镜像
- 装了代理 = 所有命令直接跑通，**省下绕路时间至少 30 分钟**

**怎么装**：用你已有的代理工具（Clash / Surge / Stash 等），或者如果你团队/朋友有现成的订阅，先在 M5 Max 上配上。

**验证**：终端里能跑通：
```bash
curl -I https://github.com    # 期望返回 HTTP/2 200
```

如果你**完全不能/不愿用代理**：跳到本教程附录的"国内镜像版替代方案"，但准备多花 30-60 分钟。

> 这节是我装到一半才意识到的事情，所以记下来——你不要再走我走过的弯路。

**最扎心的对比**：

我装 Claude Code 的真实时间记录：
